...
#   cppm: a float number showing acquisition metric comparing peak height to data average
 ...
 #   print 'iMat.shape: ' , iMat.shape
    ...
    if datacomplex:
        raw_data = raw_data['i']+1j*raw_data['q']
    ...
    for b in range(np.size(bins)): # Go through all possible Doppler shift
	    ...
	    pfft = np.fft.fft(raw_data[(k*S):((k+1)*S)]*iMat[b,(k*S):((k+1)*S)])
	...
	rawc = (raw_data-np.mean(raw_data))*r
...
#---Demonstration on writing and reading file from python. Tested------
# Test writing files

#print "Reading/writing file demo"
